...
- Pre-existing bug: test setup schema missing `input_tokens`, `output_tokens` columns (3 test failures in agent.test.ts)

### Build Verification Checklist
- TypeScript compilation: `pnpm build` (checks server + CLI)
- Vite web build: included in `pnpm build` (checks React components)
- Full test suite: `pnpm test` (currently 161 pass, 3 pre-existing failures)
...
- Hook event names must match between hooks-config.json template, hook.sh event extraction, and hooks.ts switch-case
...
- Pre-existing test issue: agents table in test setup missing input_tokens/output_tokens columns
