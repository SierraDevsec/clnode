...
6. Copy templates based on flags:
   - Default: skills + universal agents (reviewer.md, worker.md) + universal rules (clnode-usage.md)
   - `--with-agents`: all agents + all rules (for clnode development)
   - `--hooks-only`: skip all templates
...
- Init options expanded: `--hooks-only` and `--with-agents` flags added for flexibility
- Universal agents (reviewer.md, worker.md) and rules (clnode-usage.md) now installed by default
