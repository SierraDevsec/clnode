...
- All hook events handled in single switch-case in `routes/hooks.ts`
...
- Test setup schema (`src/__tests__/setup.ts`) is MISSING `input_tokens` and `output_tokens` columns on agents table; this causes 3 pre-existing test failures in `agent.test.ts`
- Queries test setup (`services/queries/__tests__/setup.ts`) has its own duplicated schema definition -- must be kept in sync with main test setup
